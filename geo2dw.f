...
c XM YU add if | later, S.N.KIM includes one more if statement (ian.eq.2). Aug. 2018
         if (ian.eq.6.or.ian.eq.2) nsw(m)=nwpanel+1
c XM YU add if | later, S.N.KIM includes one more if statement (ian.eq.2). Aug. 2018
...
C           write(83,*) N, TH, NN, THM1
 ...
 20     CONTINUE
         ...
         DO N = 1, NSUB
           IF(N.EQ.1) THEN
             DWXI = (XIVW(N,M)-XIV(1,M))/FLOAT(NWSUB1)
             DWETA = (ETAVW(N,M)-ETAV(1,M))/FLOAT(NWSUB1)
...
C       Find the middle section on the blade as the 2-D blade strip    
C       file unit 840 outputs the geometry of the 2-D strip 

      DO 40 M=1,MR
c           write(840,*) 'ZONE T= "M= ', M, ' " '
...
c            write(840,100) N,XIVIS(N,M),ETAVIS(N,M)
 5         CONTINUE
...
c             write(840,100) N+NCP,XIVIS(N+NCP,M),ETAVIS(N+NCP,M)
 15         CONTINUE

         DO 25 N=NSUB+1,NSW(M)-1
...
c             write(840,100) N1,XIVIS(N1,M),ETAVIS(N1,M)
 25         CONTINUE
40      CONTINUE

 100    format(1x,I5,2(3x,f10.5))
C 101      format(4(f10.5,3x))
c        close(840)
