...
C-----------------------------------------------------------------------
       DO 10 IDR=1,2
          ...
          IF(IDR.EQ.1) THEN
             DO M=1,MR
                IF(JCV(M,IDR).EQ.0.OR.NOCAV(M,IDXREV,IDR).EQ.1) THEN
                   DUM(M)=ZERO
                ELSE
                   CAVLTE=ARCLNG(NHP-NSR2,M,IDR)/ARCLNG(NHP,M,IDR)
                   DUM(M)=CAVL(M,IDR)/CAVLTE
                END IF
             END DO
c             WRITE(*,'(a4,1x,10(F6.3,1X))') 'Back',(DUM(M),M=1,MR)
          ELSE IF(IDR.EQ.2) THEN
             DO M=1,MR
                ...
                END IF
             END DO
c             WRITE(*,'(a4,1x,10(F6.3,1X))') 'Face',(DUM(M),M=1,MR)
             DO M=1,MR
                CAVLTE=ARCLNG(NHP-NSR2,M,IDR)/ARCLNG(NHP,M,IDR)
                DUM(M)=CAVLT(M)/CAVLTE
             END DO
c             WRITE(*,'(a4,1x,10(F6.3,1X))') 'Supr',(DUM(M),M=1,MR)
          END IF
...
C-----------------------------------------------------------------------
 ...
 30          CONTINUE
             ...
             DO N=1,NSR2
                ...
                DSN=SS(L,1)                
                NC1=NC1+N
                DHM=HALF*(HT(NC1+1,M,IDR)+HT(NC1,M,IDR))
                DELA1=DELA1+DHM*DSN
             END DO
             CVOW=CVOLW+DELA1
             CVOL=CVOL+DELA1
 ...
 10    CONTINUE
       ...
       DO 40 M=1,MR
          DELA1=ZERO
          IF(NNWC(M).GT.0) THEN
             IF(JCV(M,NWDIR(M)).GT.0) THEN
                NC1=NH-NLEP(M,IDXREV,NWDIR(M))
             ELSE
                NC1=NSR2
             END IF
...
! s--- Allen Du 04112018 couple cavopt for mercury case
       opt_vol=cvolf
