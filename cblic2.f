      ...
      SUBROUTINE CBLIC2(SIG,RHSM,TOL,NORD,NBLOCK,NPERB,
     *  ITMAX,AINF,NNR,NBLK,ITMX)
************************************************************************
...
!     PARAMETER(NNR=NTZ,NBLK=NBLKMAX,ITMX=101)
...
C-----------------------------------------------------------------------
      DO 50 NB=1,NBLOCK
        ...
        DO 40 JCOL=1,NPERB(NB)
          ...
          DO 30 I=1,NPERB(NB)
C---------Convert AD to double precision from AINF CM102897-------------
            AD(KST+I,NB)=DBLE(AINF(ID0+I,KSS))
 30       CONTINUE
 40     CONTINUE
 50   CONTINUE
C     
      ...
      DO 230 NB=2,NBLOCK
        DO 220 JM=IDSTRT(NB),IDEND(NB)
          DO 210 IROW=1,NSU(NB)
            RES(IROW)=RES(IROW)-AINF(IROW,JM)*DSIG(JM)
 210      CONTINUE
 220    CONTINUE
 230  CONTINUE
C     
...
C     
      IF(RESERR.LE.TOL) THEN
        ...
        DEALLOCATE(NPERSQ,IPVT1)
        DEALLOCATE(IPVT)
        DEALLOCATE(AD,Q11,Q12,WORK)
        DEALLOCATE(RHSDBL)
        RETURN
      END IF
C     
      IF(IT.GE.ITMAX) THEN
        ...
        RETURN
      END IF
 9050 FORMAT(/2X,'NO CONVERGENCE IN CBLIC2 AFTER',I4,
