       SUBROUTINE CAVSC2(MM,NN,IEQN,I1)
************************************************************************
*                                                                      *
...
C.......cavitating panels on face SR region
         DO N=1,N0(2)-1
            ALHS(IEQN,IFRST1+N)=-E(I1,IFRST+N)
            SUMF2=SUMF2+D(I1,IFRST+N)
         END DO
...
C.......face SR region.
         IF(ISTEADY.EQ.0) THEN
            IF(NWDIR(MM).EQ.1) THEN
               FCTF2=SUMF2-WK(I1,M)
            ELSE IF(NWDIR(MM).EQ.2) THEN
               FCTF2=SUMF2-WK2(I1,M)
            END IF
         ELSE
            FCTF2=SUMF2-HALF*WK1
         END IF
                
         IF(M.EQ.MM.AND.NWDIR(MM).EQ.2) FCTF2=FCTF2+FOURPI
...
C.......cavitating panels on back SR region
         DO N=N0(1),NC
            ...
            ALHS(IEQN,IFRST1+N123)=-E(I1,IFRST+N)
            SUMB2=SUMB2+D(I1,IFRST+N)
         END DO
...
C.......back SR region.
         IF(ISTEADY.EQ.0) THEN
            IF(NWDIR(MM).EQ.1) THEN
               FCTB2=SUMB2+WK(I1,M)
            ELSE IF(NWDIR(MM).EQ.2) THEN
               FCTB2=SUMB2+WK2(I1,M)
            END IF
         ELSE
            FCTB2=SUMB2+HALF*WK1
         END IF

         IF(M.EQ.MM.AND.NWDIR(MM).EQ.1) FCTB2=FCTB2+FOURPI
...
C.......cavitating panels on face SR region
         DO N=1,N0(2)-1
            N123=N0(2)-N
            SUM2=SUM2+D(I1,IFRST+N)*PHI2(N123,M,2)
         END DO
...
C.......only if wetted or partially cavitating
         ...
         ELSE
C.......only if wetted or supercavitating
...
C.......cavitating panels on back SR region
         DO N=N0(1),NC
            N123=N-N0(1)+1
            SUM2=SUM2+D(I1,IFRST+N)*PHI2(N123,M,1)
         END DO
