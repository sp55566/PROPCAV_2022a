       SUBROUTINE CAVSC2_SP(MM,NN,IEQN,I)
************************************************************************
*                                                                      *
...
*  for equation IEQN for the given column I = function (NN,MM).        *
...
CVV
      ALLOCATE(WWK2(NPAWZ,NPAWZ))
CVV

      IF(NTSTEP.EQ.1) THEN
         REWIND 91
         IF(IMG.EQ.1) REWIND 191
         DO M=MR,1,-1
            DO L=1,NSUB
               J=IDXWAK(L,M)
               CALL READ1(91,TEMP4,NPWAKS)
               IF(IMG.EQ.1) CALL READ1(191,TEMP3,NPWAKS)
               DO I1=1,NPWAKS
                  IF(L.LE.MSW(M,IDXREV)) THEN
                     IF(IMG.EQ.0) THEN
                        WWK2(I1,J)=TEMP4(I1)
                     ELSE
                        WWK2(I1,J)=TEMP4(I1)-TEMP3(I)
                     END IF
                  ELSE
                     WWK2(I1,J)=ZERO
                  END IF
...
C.............wettted panels on the face
               DO 120 N=IW(1,M,IDXREV),IW(2,M,IDXREV)
                  JSUBM=JSUBM+1
                  ALHS(IEQN,IFRST1+JSUBM)=D(I,IFRST+N)
 120           CONTINUE
            ELSE
...
C................cavitating panels on face SR region
                  DO N=IW(1,M,IDXREV),N0(2)-1
                     JSUBM=JSUBM+1
                     ALHS(IEQN,IFRST1+JSUBM)=-E(I,IFRST+N)
                     FCTF2=FCTF2+D(I,IFRST+N)
                  END DO
...
C................term associated with Kutta condition
                  IF(IW(1,M,IDXREV).EQ.1) THEN
                     IF(NTSTEP.EQ.1) THEN
                        DO L=1,MSW(M,IDXREV)
                           FCTF2=FCTF2-WWK2(I,IDXWAK(L,M))
                        END DO
                     ELSE
                        FCTF2=FCTF2-HALF*WK(I,M)
                     END IF
                  END IF
               END IF
...
C............cavitating panels on back SR region
               ...
               DO N=N0(1),IC(2,M,IDXREV)                  
                  NJ=NJ+1
                  ALHS(IEQN,IFRST1+NJ)=-E(I,IFRST+N)
                  FCTB2=FCTB2+D(I,IFRST+N)
               END DO
...
C.............term associated with Kutta condition
               IF(IC(2,M,IDXREV).EQ.NC) THEN
                  IF(NTSTEP.EQ.1) THEN
                     DO L=1,MSW(M,IDXREV)
                        FCTB2=FCTB2+WWK2(I,IDXWAK(L,M))
                     END DO
                  ELSE
                     FCTB2=FCTB2+HALF*WK(I,M)
                  END IF
                  
                  IF(M.EQ.MM.AND.NNWC(M).GT.0) FCTB2=FCTB2+FOURPI
...
C.............term associated with Kutta condition
     ...
     *              AND.SOP(M,1).EQ.ONE) THEN
                  IF(NTSTEP.EQ.1) THEN
                     DO L=1,MSW(M,IDXREV)
                        FCTB=FCTB+WWK2(I,IDXWAK(L,M))
                     END DO
                  ELSE
                     FCTB=FCTB+HALF*WK(I,M)
                  END IF
                  IF(M.EQ.MM.AND.NNWC(M).GT.0) FCTB=FCTB+FOURPI
               END IF
...
C.......submerged panels on the face side
         IF(ICB(M,2,IDXREV).EQ.1) THEN
            ...
            IF(ISC.EQ.0) THEN
C.............wettted panels on the face
               DO 150 N=IW(1,M,IDXREV),IW(2,M,IDXREV)
                  N1=IFRST+N
                  SUM3=SUM3+E(I,N1)*DPDNC(N1)
 150           CONTINUE
...
C................cavitating panels on face SR region
                  DO N=IW(1,M,IDXREV),N0(2)-1
                     N123=N0(2)-N
                     SUM2=SUM2+D(I,IFRST+N)*PHI2(N123,M,2)
                  END DO
...
C.............cavitating panels on back SR region
               DO N=N0(1),IC(2,M,IDXREV)
                  N123=N-N0(1)+1
                  SUM2=SUM2+D(I,IFRST+N)*PHI2(N123,M,1)
               END DO
...
C.............Cavitating panels on the back side
               ...
               DO 160 N=M0M,NLAST
                  ICAV=ICAV+1
                  SUM2=SUM2+D(I,IFRST+N)*PHI1(ICAV,M,1)
 160           CONTINUE
...
C.......term associated with Kutta condition
 ...
 110  CONTINUE
      ...
      IF(ISC.EQ.0) THEN
         JCAV=JCV(MM,1)
         RHS(IEQN)=RHS(IEQN)-FOURPI*PHI1(JCAV+NN,MM,1)
      ELSE
         RHS(IEQN)=RHS(IEQN)-
     *        FOURPI*PHI2(NSR2+NN,MM,NWDIR(MM))
      END IF
