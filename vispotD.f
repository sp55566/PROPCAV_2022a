...
C.............Transfer control points to local coordinate...............
          ...
          ZLOC=ZERO
          DO K=1,3
             XLOC=XLOC+(XCTPDWs(L,K,KK)-XCT(J,K))*DIR(J,1,K)
             YLOC=YLOC+(XCTPDWs(L,K,KK)-XCT(J,K))*DIR(J,2,K)
             ZLOC=ZLOC+(XCTPDWs(L,K,KK)-XCT(J,K))*DIR(J,3,K)
          ENDDO  
...
c         Calculate Total potential
          PPOTDWs(L) = PPOTDWs(L)+ (-POT(J)*FD +DPDN(J)*FS)/PI/4.
...
C.............Transfer control points to local coordinate...............
          ...
          ZLOC=ZERO
          DO K=1,3
             XLOC=XLOC+(XCTPDW(L,K,KK)-XCT(J,K))*DIR(J,1,K)
             YLOC=YLOC+(XCTPDW(L,K,KK)-XCT(J,K))*DIR(J,2,K)
             ZLOC=ZLOC+(XCTPDW(L,K,KK)-XCT(J,K))*DIR(J,3,K)
          ENDDO  
...
c         Calculate Total potential
          PPOTDW(L) = PPOTDW(L)+ (-POT(J)*FD +DPDN(J)*FS)/PI/4.
...
C
      DO 560 M = 1, MDUCT 
       DO 550 N = 1, NWSUB
        ...
        JMAC=INDEXWD(JSUB,M)
        JMAC1=INDEXWD(JSUB+1,M)
        
        SLOPE=(TEMPD4(JMAC1)-TEMPD4(JMAC)) / (XDW(JSUB+1,M)-XDW(JSUB,M))
        TEMPSUB(J,KK) = SLOPE*(XCTPDWs(J,1,KK)-XDW(JSUB,M))+TEMPD4(JMAC)
...
C.............Transfer control points to local coordinate...............
          ...
          ZLOC=ZERO
          DO K=1,3
             XLOC=XLOC+(XCTPDWs(L,K,KK)-XCTW(J,K))*DIRW(J,1,K)
             YLOC=YLOC+(XCTPDWs(L,K,KK)-XCTW(J,K))*DIRW(J,2,K)
             ZLOC=ZLOC+(XCTPDWs(L,K,KK)-XCTW(J,K))*DIRW(J,3,K)
          ENDDO  
...
c         Calculate Total potential
          ...
          IF(KK.EQ.1) PPOTDWs(L)=PPOTDWs(L)-TEMPSUB(J,KK)*FD/PI/4.
...
C.............Transfer control points to local coordinate...............
          ...
          ZLOC=ZERO
          DO K=1,3
             XLOC=XLOC+(XCTPDWs(L,K,KK)-XCTWD(J,K))*DIRWD(J,1,K)
             YLOC=YLOC+(XCTPDWs(L,K,KK)-XCTWD(J,K))*DIRWD(J,2,K)
             ZLOC=ZLOC+(XCTPDWs(L,K,KK)-XCTWD(J,K))*DIRWD(J,3,K)
          ENDDO  
...
c         Calculate Total potential
         IF(KK.EQ.1.AND.N.GT.NSUB) THEN
           PPOTDWs(L) = PPOTDWs(L)- TEMPD4(J)*FD/PI/4.
         ELSE IF(KK.GT.1) THEN
           PPOTDWs(L) = PPOTDWs(L)- TEMP5(J)*FD/PI/4.       
         ENDIF
 630    CONTINUE
...
C.............Transfer control points to local coordinate...............
          ...
          ZLOC=ZERO
          DO K=1,3
             XLOC=XLOC+(XCTPDW(L,K,KK)-XCTWD(J,K))*DIRWD(J,1,K)
             YLOC=YLOC+(XCTPDW(L,K,KK)-XCTWD(J,K))*DIRWD(J,2,K)
             ZLOC=ZLOC+(XCTPDW(L,K,KK)-XCTWD(J,K))*DIRWD(J,3,K)
          ENDDO  
...
c         Calculate Total potential          
         IF(KK.EQ.1) THEN
            PPOTDW(L) = PPOTDW(L)- TEMPD4(J)*FD/PI/4.
         ELSE 
            PPOTDW(L) = PPOTDW(L)- TEMP5(J)*FD/PI/4.       
         ENDIF
...
c         Calculate Total potential
         IF(KK.EQ.1) THEN
          PPOTDWs(L) = PPOTDWs(L)- TEMP4(J)*FD/PI/4.
         ELSE 
          PPOTDWs(L) = PPOTDWs(L)- TEMP5(J)*FD/PI/4.       
         ENDIF
 830    CONTINUE
...
C.............Transfer control points to local coordinate...............
          ...
          ZLOC=ZERO
          DO K=1,3
             XLOC=XLOC+(XCTPDW(L,K,KK)-XCTW(J,K))*DIRW(J,1,K)
             YLOC=YLOC+(XCTPDW(L,K,KK)-XCTW(J,K))*DIRW(J,2,K)
             ZLOC=ZLOC+(XCTPDW(L,K,KK)-XCTW(J,K))*DIRW(J,3,K)
          ENDDO  
...
c         Calculate Total potential          
         IF(KK.EQ.1) THEN
           PPOTDW(L) = PPOTDW(L)- TEMP4(J)*FD/PI/4.
         ELSE 
           PPOTDW(L) = PPOTDW(L)- TEMP5(J)*FD/PI/4.       
         ENDIF
