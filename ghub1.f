...
C-----------------------------------------------------------------------
      ...
      DO 90 N=1,NHBDT
         ...
         TH(NN,1)=THW
         RNOS=RNOSE( (XHBFD-XW(N,1))/XHBT,RHUB )
...
C         IF(RNOS.LT.RHULT) THEN
C            RR=RNOS
C            YH(NN,1)=RR*COS(THW)
C            ZH(NN,1)=RR*SIN(THW)
C         ELSE
C            RR=SQRT(YW(N,1)**2+ZW(N,1)**2)
C            YH(NN,1)=YW(N,1)
C            ZH(NN,1)=ZW(N,1)
C         END IF
...
100   CONTINUE
C-----------------------------------------------------------------------
C     Hub at far upstream of the blade is replaced by a dipole disk
C-----------------------------------------------------------------------
      NHBDK=MHBT
      DTH=DELK/NHBDK
      DO 110 N=1,NHBDK+1
